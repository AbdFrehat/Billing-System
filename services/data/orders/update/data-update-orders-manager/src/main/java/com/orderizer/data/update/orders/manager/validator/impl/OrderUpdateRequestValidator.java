...
@Component
public class OrderUpdateRequestValidator implements Validator<OrderUpdateRequest> {
    @Override
    public Mono<OrderUpdateRequest> validate(OrderUpdateRequest order) {
        return Mono.defer(() -> {
            if (order.getLocalIdentifier() < 0L) {
                return Mono.error(new RequestBodyInvalidException("Local Identifier cannot be negative."));
            }
